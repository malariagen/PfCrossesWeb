...
CREATE TABLE `variant_filter` (
  ...
  KEY `ifilt` (`filter`),
  KEY `imethod` (`method`),
  KEY `ifiltmethod` (`filter`,`method`),
  CONSTRAINT `fk_variant_filter_1` FOREIGN KEY (`variant_id`) REFERENCES `pfx_variants` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB;

INSERT INTO variant_filter (variant_id, filter, method) SELECT pv.id, pf.filter,pv.method FROM pfx_variants pv
...
INSERT INTO variant_filter (variant_id, filter, method)
SELECT pv.id, pf.filter, pv.method FROM pfx_variants pv
    JOIN pfx_variant_filter pf ON 
